...
class Egg(pg.sprite.Sprite):
    def __init__(self, display_surf, pos, group):
...
class AssemblyLine:
    ...
    def update(self):
        ...
        self.display.blit(self.img, (WIN_WIDTH - 12, 0))
...
class Bucket(pg.sprite.Sprite):
    def __init__(self, display, group):
        ...
        self.milked_surf = pg.image.load(
            "assets/cow/buckets/milked.png"
        ).convert_alpha()
        self.surf = self.empty_surf
        self.rect = self.surf.get_rect()

        self.pos = pg.Vector2(WIN_WIDTH - 6, 0)
    ...
    def grab(self, mpressed, mpos, cursor_rect):
        if not self.milked:
            if mpressed[0]:
                ...
                if self.pos.x > WIN_WIDTH - 18:
                    self.pos.x = WIN_WIDTH - 6
                    self.pos.y += 0.4
