...
class Chicken(pg.sprite.Sprite):
    def __init__(self, display_surf, group):
        ...
        self.pos = pg.Vector2(random.randint(0, WIDTH) - (self.size[0] / 2), random.randint(0, HEIGHT) - (self.size[1] / 2))
        
    def sprite_import(self, filename):
        
        sprite = pg.transform.flip(pg.image.load(f'assets/chicken/{filename}.png').convert_alpha(), True, False)
    ...
    def update(self):
        ...
        elif self.rect.right > WIDTH - 4:
            self.pos.x = WIDTH - self.size[0] - 4
            self.x_speed *= -1
            self.surf = pg.transform.flip(self.surf, True, False)
        elif self.rect.bottom > HEIGHT - 4:
            self.pos.y = HEIGHT - self.size[1] - 4
            self.y_speed *= -1
...
class Sheep(pg.sprite.Sprite):
    ...
    def __init__(self, display, spawn_pos, group):
        ...
        self.speed = 0.25
        self.radius = 1.5
        self.pos = pg.Vector2(spawn_pos[0] - (self.size[0] / 2), spawn_pos[1] - (self.size[1] / 2))
