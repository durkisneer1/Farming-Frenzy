...
class ShpStats:
    def __init__(self, display, max_sheep):
        self.display = display
        self.max_sheep = max_sheep

        self.font = pg.font.Font("fonts/m5x7.ttf", 15)
        self.shp_count_surf = self.font.render(f"0/{self.max_sheep}", False, "white")
        self.shp_count_rect = self.shp_count_surf.get_rect(topleft=(2, 0))

    def shp_count(self, collected, max_sheep):
        self.max_sheep = max_sheep
        self.shp_count_surf = self.font.render(
            f"{collected}/{self.max_sheep}", False, "white"
        )
...
class Button:
    ...
    def update(self):
        pg.draw.rect(
            self.display,
            "white",
            pg.Rect((self.pos[0] - 2, self.pos[1] - 1), self.txt_size),
        )
        pg.draw.rect(
            self.display,
            "black",
